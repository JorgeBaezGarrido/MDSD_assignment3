...
// Entry rule entryRuleGivenString
entryRuleGivenString returns [String current=null]:
	{ newCompositeNode(grammarAccess.getGivenStringRule()); }
	iv_ruleGivenString=ruleGivenString
	{ $current=$iv_ruleGivenString.current.getText(); }
	EOF;

// Rule GivenString
ruleGivenString returns [AntlrDatatypeRuleToken current=new AntlrDatatypeRuleToken()]
@init {
	enterRule();
}
@after {
...
@after {
	...
	(
		...
		{
			$current = $this_Exponent_4.current;
			afterParserOrEnumRuleCall();
		}
	)
;
